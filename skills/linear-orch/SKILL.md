---
name: linear-orch
description: "PRIMARY/MAIN AGENT ONLY — do NOT load as a sub-agent. Per-issue Linear lifecycle orchestrator (dev → review → submit → merge) that spawns specialist sub-agents and review pipelines from inside a worktree."
license: MIT
user-invocable: true
dependencies:
...
2. Otherwise (running from main repo) → emit a redirect message (`From main, use 'flightdeck linear start [ISSUE_ID]' — that command lives in the flightdeck skill.`) and stop. linear-orch's role is per-issue inside-worktree work; master-side kickoff lives in flightdeck.
...
```bash
.agents/skills/linear-orch/scripts/<script> [args]
```
...
`bash skills/linear-orch/tests/run-all.sh` runs every script-level regression test (`bot_review_wait.sh`, `ci_wait.sh`, `session_init.sh`, and related helpers). Tests stage isolated repos/worktrees with parametrized CLI stubs on `PATH`; the auth tests exercise stale-token sanitize, keyring fallback, `.env.local` `GH_BOT_TOKEN` fallback, hard “no working auth path” exit, and worktree Linear auth diagnostic preservation.
...
Run the full linear-orch regression suite:
