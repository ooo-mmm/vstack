...
Terminal issue panes are closed by `workflows/close-issue.md` after the two-signal terminal-state check updates master state; registry entries remain until `terminate.md` writes the final report and runs final cleanup. Non-terminal panes remain under user control; flightdeck does not respawn dead panes or manage panes outside the current session.
