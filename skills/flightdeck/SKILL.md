...
| `pane-poll` | Single-window status read. Accepts either legacy `<session>:<window> <pane-index>` or immutable `%N` pane ids directly (preferred from registry `pane_id`, survives pi/codex window renames). Per-harness adapter routes: opencode → `GET /session/<id>/message`; claude → tail of `~/.claude/projects/<encoded-cwd>/<uuid>.jsonl`; pi → `pi-bridge history`; codex → `codex-bridge turns`. Tmux `capture-pane` is the fallback when a pane has no bridge metadata. Bell flag + classify. |
