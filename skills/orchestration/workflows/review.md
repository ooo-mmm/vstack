...
**Detect team context:**
```bash
TEAM=$(.agents/skills/orchestration/scripts/workflow-state get $ISSUE_ID '.team_name // empty' 2>/dev/null) || true
```
...
1. **Capture pre-fix state** (if `ISSUE_ID` exists):
   ```bash
   .agents/skills/orchestration/scripts/workflow-state set [ISSUE_ID] pre_delegate_sha "$(git rev-parse HEAD)"
   ```
...
3. **Update state** (if `ISSUE_ID` exists):
   ...
   .agents/skills/orchestration/scripts/workflow-state append [ISSUE_ID] fixed_items '{"description":"[DESC]","location":"[LOC]","commit":"[SHA]","source":"review"}'

   # For each escalated/skipped item:
   .agents/skills/orchestration/scripts/workflow-state append [ISSUE_ID] escalated_items '{"description":"[DESC]","location":"[LOC]","reason":"[REASON]","source":"review"}'
   ```
